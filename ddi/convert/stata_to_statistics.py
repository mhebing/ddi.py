...
def stata_to_statistics(study_name, input_csv, input_path, output_path):
    ...
    for data, weight, split, analysis_unit, period, sub_type in zip(filereader.filename, filereader.weight, filereader.split, filereader.analysis_unit, filereader.period, filereader.sub_type):
        ...
        try:
            ...
            continue
        d1.write_tdp(output_path + data + ".csv", output_path + data + ".json")
        d1.write_stats(output_path + data + "_stats.json", split=split, weight=weight, analysis_unit=analysis_unit, period=period, sub_type=sub_type, study=study_name)
