...
class Parser:
    """
    Parser for DDI-based XML format, generated by r2ddi.

    Example: if you want to parse the SOEPcore XML files for version v33, the
    English version of the files is expected to be stored in the directory
    ``r2ddi/v33/en`` and the German files in ``r2ddi/v33/de``. Then run the
    following command to generate the json files for DDI on Rails 2::

        from ddi.onrails.repos.convert_r2ddi import Parser
        Parser("soep-core", versions=["v33"], latest_version="v33").write_json()
    """
    ...
    def __init__(self, study_name, r2ddi_path="r2ddi", version=None, primary_language="en",
        ...
        """
        The ``version`` option is now DEPRECATED, please use the combination of
        ``versions`` (list) and ``latest_version`` from now on.

        Parameters:

        study_name
            Name of the study, e.g., ``soep-core``

        r2ddi_path
            Directory, where the XML files from r2ddi are stored

            Default: r2ddi

        version
            DEPRECATED Version of the data to use (defines the sub-directory to
            use).

            Default: None (fine, becaus the parameter is deprecated)

        primary_language
            Language to be used as the default language (defines the
            sub-directory to use).

            Default: en

        versions
            To be implemented: combine multible versions in one output file.

            Default: ["v1"]

        latest_version
            Version to be used as the default version.

            Default: "v1"

        datasets_csv
            Path to the ``datasets.csv`` file.

            Default: "ddionrails/datasets.csv")
        """
    ...
    def write_json(self):
        """
        Delete existing json files in the ``ddionrails/datasets/`` directory
        and write the data from the parser to this directory.
        """
